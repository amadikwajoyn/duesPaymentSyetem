...
if(isset($_POST['checkboxes'])){
    ...
    foreach($_POST['checkboxes'] as $user_id){
        ...
        if($bulk_option == 'delete'){
            ...
            $bulk_author_query = "UPDATE `payments` SET `status` = 'approve' WHERE `payments`.`id` = $user_id";
            mysqli_query($con, $bulk_author_query);
        }
        else if($bulk_option == 'pending'){
            $bulk_admin_query = "UPDATE `payments` SET `status` = 'pending' WHERE `payments`.`id` = $user_id";
